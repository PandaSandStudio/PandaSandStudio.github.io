...
class TerrainGenerator {
  constructor({
    ...
    this.arrayBuffers = {
      vertices: new ArrayBuffer(
        arrayBufferLength * Float32Array.BYTES_PER_ELEMENT
      ),
  ...
  generateVertices() {
    const { vertices, lacunarity, persistance, octaves } = this;
    ...
    for (let j = 0; j < this.width; j++) {
      for (let i = 0; i < this.depth; i++) {
        ...
        for (let k = 0; k < octaves; k++) {
          ...
          const withCurveApplied = this.evaluateHeightCurve(
            this.heightCurve,
            normalizedValue
          );
  ...
  flattenVertices() {
    const { vertices, width, depth } = this;
  ...
  generateColors() {
    const { colors, vertices, regions } = this;
          ...
          if (lastRegion) {
            const blend = Math.min(
              (height - lastRegion.height) /
                ((region.height - lastRegion.height) * region.blend),
              1
            );

            const r =
              lastRegion.color[0] +
              (region.color[0] - lastRegion.color[0]) * blend;
            const g =
              lastRegion.color[1] +
              (region.color[1] - lastRegion.color[1]) * blend;
            const b =
              lastRegion.color[2] +
              (region.color[2] - lastRegion.color[2]) * blend;
